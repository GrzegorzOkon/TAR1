...
public class ReadingResult {
    ...
    private String sql = "(SELECT 'TT_ADDCDTYPE_MEASURETYPE' as tabela, COUNT(*) as ilosc FROM TT_ADDCDTYPE_MEASURETYPE) union all" +
    ...
    public Map<String, Object[]> executeQuery(Connection ActiveConnection) {
        // Zmienne dla wartości przechowywanych w tabeli TT_centr
        String column_tabela = "";
        Integer column_ilosc = 0;

        try {
            // Wykonanie SQL
            pstmt = ActiveConnection.prepareStatement(sql, ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
            //pstmt.setString();

            rs = pstmt.executeQuery();

            while (rs.next()) {
                column_tabela = rs.getString("tabela");
                column_ilosc = rs.getInt("ilosc");

                data.put(column_tabela.replace("_", " "), new Object[] {column_tabela, column_ilosc});
            }
